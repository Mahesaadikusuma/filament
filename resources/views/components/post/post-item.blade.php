...
<article class="[&:not(:last-child)]:border-b border-gray-100 pb-10">
    <div class="article-body grid grid-cols-12 gap-3 mt-5 items-start">
        ...
        <div class="col-span-8">
            <div class="article-meta flex py-1 gap-2 text-sm items-center">

                @if ($post->author->profile_photo_path)
                    <img class="h-8 w-8 rounded-full object-cover"
                        src="{{ Storage::url($post->author->profile_photo_path) }}" alt="{{ $post->author->name }}" />
                @else
                    <img class="h-8 w-8 rounded-full object-cover"
                        src="https://ui-avatars.com/api/?name={{ $post->author->name }}&background=random&color=ffff"
                        alt="{{ $post->author->name }}">
                @endif
                {{-- <img class="w-7 h-7 rounded-full mr-3" src="{{ $post->author->profile_photo_path }}"
